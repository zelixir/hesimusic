...
class PlaybackPreferences @Inject constructor(
    ...
    fun getQueue(): List<Long> {
        ...
        if (idsString.isEmpty()) {
            Log.d(TAG, "getQueue: retrieved 0 songs")
            return emptyList()
        }
        return try {
    ...
    fun getPlaylistContext(): PlaylistContext? {
        val typeName = prefs.getString(KEY_PLAYLIST_TYPE, null) ?: run {
            Log.d(TAG, "getPlaylistContext: type=null, value=")
            return null
        }
    ...
    /**
     * Consume (get and clear) the saved playlist context.
     * Used by UI to restore state on app startup without continuously re-navigating.
     */
    fun consumeSavedPlaylistContext(): PlaylistContext? {
        val context = getPlaylistContext()
        if (context != null) {
            // Clear the saved context so we don't navigate again on next composition
            prefs.edit()
